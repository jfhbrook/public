...
exports.honeycomb = exports.resetOtelMockSpans = exports.OtelMockSpanProcessor = exports.getOtelMockSpans = exports.createMockHoneycomb = exports.HoneycombError = exports.Honeycomb = exports.defaultOtelFactories = exports.OtelPgInstrumentation = exports.OtelRedisInstrumentation = exports.OtelHttpInstrumentation = exports.OtelDnsInstrumentation = exports.otelSemanticConventions = exports.NodeTracerProvider = exports.otelTraceBase = exports.OtelSDK = exports.otelResources = exports.otelCore = exports.otel = exports.beeline = exports.isDev = exports.bole = void 0;
/**/
...
exports.OtelMockSpanProcessor = OtelMockSpanProcessor;
function getOtelMockSpans(spanProcessor) {
    const processor = spanProcessor;
    if (!processor) {
        throw new Error('Span processor is not defined - did you initialize honeycomb?');
    }
    if (!processor._exporterCreatedSpans) {
        throw new Error('Span processor is not an OtelMockSpanProcessor');
    }
    return (processor._exporterCreatedSpans);
}
exports.getOtelMockSpans = getOtelMockSpans;
function resetOtelMockSpans(spanProcessor) {
    const processor = spanProcessor;
    if (!processor) {
        throw new Error('Span processor is not defined - did you initialize honeycomb?');
    }
...
exports.resetOtelMockSpans = resetOtelMockSpans;
function createMockHoneycomb() {
...
exports.createMockHoneycomb = createMockHoneycomb;
void ``;
...
exports.honeycomb = honeycomb;
void ``;
exports.honeycomb = honeycomb = createMockHoneycomb();
void ``;
honeycomb.init();
void ``;
...
if (require.main === module) {
    const { test } = tap_1.default;
    test('Honeycomb.parseEnv', (t) => __awaiter(void 0, void 0, void 0, function* () {
        t.test('options.disable', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.equal(Honeycomb.parseEnv({}).disable, true, 'should be disabled when no env vars');
            assert.equal(Honeycomb.parseEnv({ HONEYCOMB_WRITEKEY: '' }).disable, true, 'should be disabled when env vars are blank');
            assert.equal(Honeycomb.parseEnv({ HONEYCOMB_WRITEKEY: 'some write key' }).disable, false, 'should be enabled when write key is defined');
            assert.equal(Honeycomb.parseEnv({ OTEL_ENABLED: '1' }).disable, false, 'should be enabled if any otel env var is set');
        }));
        t.test('options.otel', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.equal(Honeycomb.parseEnv({}).otel, false, 'should not use otel when no env vars');
            assert.equal(Honeycomb.parseEnv({
                HONEYCOMB_WRITEKEY: '',
                HONEYCOMB_API_HOST: 'https://refinery.tech'
            }).otel, false, 'should not use otel when only beeline variables are set');
            assert.equal(Honeycomb.parseEnv({
                HONEYCOMB_WRITEKEY: 'some write key',
                OTEL_EXPORTER_OTLP_ENDPOINT: 'https://refinery.website'
            }).otel, true, 'should use otel when OTEL_EXPORTER_OTLP_ENDPOINT is defined');
            assert.equal(Honeycomb.parseEnv({
                OTEL_ENABLED: '1'
            }).otel, true, 'should use otel when any OTEL_* variable is defined, even if HONEYCOMB_WRITEKEY is missing');
            assert.equal(Honeycomb.parseEnv({
                OTEL_ENABLED: ''
            }).otel, false, 'should NOT use otel when all OTEL_* variables are defined as empty strings');
        }));
        t.test('options.sampleRate', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.equal(Honeycomb.parseEnv({}).sampleRate, 1, 'should be 1 by default');
            assert.equal(Honeycomb.parseEnv({ HONEYCOMB_SAMPLE_RATE: '1' }).sampleRate, 1, 'should be 1 if defined as 1');
            assert.equal(Honeycomb.parseEnv({
                HONEYCOMB_SAMPLE_RATE: '0.5'
            }).sampleRate, 0.5, 'should be 0.5 if defined as 0.5');
            assert.equal(Honeycomb.parseEnv({ HONEYCOMB_SAMPLE_RATE: 'pony' }).sampleRate, 1, 'should be 1 if not parseable');
        }));
        t.test('options.serviceName', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.equal(Honeycomb.parseEnv({}).serviceName, 'telemetrence', 'should fall back to "telemetrence" when no env vars (nor package.json)');
            assert.equal(Honeycomb.parseEnv({ SERVICE_NAME: 'test-app' }).serviceName, 'test-app', 'should use SERVICE_NAME when defined');
            ...
            }).serviceName, 'otel-test-app', 'OTEL_SERVICE_NAME should take precedence over SERVICE_NAME');
            assert.equal(Honeycomb.parseEnv({
                SERVICE_NAME: 'test-app',
                OTEL_SERVICE_NAME: ''
            }).serviceName, 'test-app', "OTEL_SERVICE_NAME should be ignored when it's a blank string");
        }));
    }));
    test('factories', (t) => __awaiter(void 0, void 0, void 0, function* () {
        t.test('headers', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            const headers = defaultOtelFactories.headers('some write key', 'some dataset');
            assert.same(headers, {
                'x-honeycomb-team': 'some write key',
                'x-honeycomb-dataset': 'some dataset'
            }, 'should have the expected headers');
        }));
        t.test('resource', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.doesNotThrow(() => defaultOtelFactories.resource('test-service'));
        }));
        test('tracerProvider', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            const resource = defaultOtelFactories.resource('test-service');
            assert.doesNotThrow(() => defaultOtelFactories.tracerProvider(resource), 'should create a tracer provider');
        }));
        test('spanExporter', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            const headers = defaultOtelFactories.headers('some write key', 'some dataset');
            process.env.OTEL_LOG_LEVEL = 'error';
        ...
        test('spanProcessor', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            assert.doesNotThrow(() => {
                const exporter = defaultOtelFactories.spanExporter('http/protobuf', defaultOtelFactories.headers('some write key', 'some dataset'));
                defaultOtelFactories.spanProcessor(exporter);
            }, 'should create a span processor');
        }));
        test('instrumentations', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            // expected instrumentations: dns, node core, postgres, redis
            assert.equal(defaultOtelFactories.instrumentations().length, 4, 'should create 4 instrumentations (dns, http, postgres, redis)');
        }));
        test('sdk', (assert) => __awaiter(void 0, void 0, void 0, function* () {
            // run the init function
            assert.doesNotThrow(() => {
                const resource = defaultOtelFactories.resource('test-service');
                const instrumentations = defaultOtelFactories.instrumentations();
                defaultOtelFactories.sdk(resource, instrumentations);
            }, 'should create an sdk');
        }));
    }));
    test('init and start', (assert) => __awaiter(void 0, void 0, void 0, function* () {
        const honeycomb = createMockHoneycomb();
        honeycomb.init();
        assert.doesNotThrow(() => __awaiter(void 0, void 0, void 0, function* () {
            yield honeycomb.start();
            yield honeycomb.stop();
        }));
    }));
}
void ``;
